# Compiler warning configuration helper

option(SRVN_WARNINGS_AS_ERRORS "Treat compiler warnings as errors" OFF)

function(set_target_warnings target)
    if(NOT TARGET ${target})
        message(FATAL_ERROR "set_target_warnings called on non-target '${target}'")
    endif()

    if(SRVN_COMPILER_MSVC)
        target_compile_options(${target} PRIVATE
            $<$<BOOL:${SRVN_WARNINGS_AS_ERRORS}>:/WX>
    ...
    if(SRVN_COMPILER_GCC OR SRVN_COMPILER_CLANG)
        target_compile_options(${target} PRIVATE
            $<$<BOOL:${SRVN_WARNINGS_AS_ERRORS}>:-Werror>
            -Wall
    ...
    if(SRVN_COMPILER_GCC)
        target_compile_options(${target} PRIVATE
            -Wmisleading-indentation # warn if indentation implies blocks where blocks do not exist
            -Wduplicated-cond  # warn if if / else chain has duplicated conditions
            -Wlogical-op # warn about logical operations being used where bitwise were probably wanted
            $<$<VERSION_GREATER_EQUAL:${CMAKE_CXX_COMPILER_VERSION},8.1>:-Wduplicated-branches>
        )
    endif()

    if(SRVN_COMPILER_CLANG OR SRVN_COMPILER_CLANG_CL)
        target_compile_options(${target} PRIVATE
